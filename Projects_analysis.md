...
## Актуальное состояние (после Фаз 1–6, версия 2.7.0)

Все пункты «Найденные баги» (§7) и «Рекомендации» (§9) выполнены в рамках DEVELOPMENT_PLAN.md.
Ключевые изменения относительно текста ниже:

- **Скачивание унифицировано** (`src/download.js`): везде `GM_download`; `saveAs`/FileSaver убраны, CDN `@require` удалён из метаданных.
- **Клик по элементу** — быстрая загрузка; **кнопка-дискета** на элементе — «Сохранить как...» (`saveAs: true`). Медиа-строки скачивают файл, а не открывают вкладку.
- **Картинки сортируются по разрешению** (W×H, по убыванию, прогрессивно); битые — заглушкой, в счётчик/«Скачать все» не входят.
- **Цвета**: поддерживаются все форматы (rgb/rgba/hsl/имена через canvas), альфа сохраняется (`#rrggbbaa`).
- **CSS полностью изолирован** под `#isf-root` (специфичность + сброс потомков).
- **Производительность**: пошаговый обход DOM через `src/scan.js` (yield каждые 300 элементов, без `#isf-root` и скрытых узлов).
- **UX**: спиннер поиска, пустые состояния по категориям; кнопка «Свернуть» удалена.
- **Мёртвый код удалён**: `config.primaryColor`, `state.drag`, IIFE-обёртка; magic-числа (ширина попапа, z-index toast, дефолтная позиция) — в `config.js`.
- **Версия** — единый источник в `package.json` (2.7.0), userscript читает её через `vite.config.js`.
- Новые модули: `src/download.js`, `src/scan.js`. Добавлены `README.md` и смоук-тест `npm run check`.
